...
class App extends Component {
  ...
  displayText(inputType, fromResults = false) {
    ...
    if (inputType === "redoWords") {
      for (let i = 0; i < 19; i++) {
        contentText +=
          this.state.incorrectWordsArr[
            Math.floor(Math.random() * this.state.incorrectWordsArr.length)
          ] + " ";
      }
      contentText +=
        this.state.incorrectWordsArr[
    ...
    if (inputType === "gen") {
      ...
      for (let i = 1; i < 100; i++) {
        if (i % 5 === 0) {
          ...
          contentText += genInputNode.value.charAt(
            Math.floor(Math.random() * genInputNode.value.length)
          );
        }
      }
      genInputNode.blur();
    ...
    this.setState({
      ...
      correctLetterCase:
        contentText.charAt(0) !== contentText.charAt(0).toLowerCase()
          ? "uppercase"
          : "lowercase",
  ...
  handleKeyPress(e) {
    ...
    if (
      ...
      if (progress === 0 && timeIncreasing === false) {
        this.intervalId = setInterval(
          function () {
            ...
            if (
              (currentCount === 59 && inputSelected === "test1") ||
      ...
      if (this.state.inputSelected === "num") {
        this.setState({
          keyPressed:
            e.key === "Shift" ? e.code : e.key === "Enter" ? " " : e.key,
          keyCode: e.keyCode,
        });
      } else {
        this.setState({
      ...
      if (keyPressed === textLetter) {
        ...
        this.setState({
          ...
          correctLetterCase:
            ...
            inputText.charAt(progress + 1).toLowerCase()
              ? "uppercase"
              : "lowercase",
          progress: progress + 1,
        });
          ...
          accuracy: String(
            (
              (this.state.completedText.length /
                (this.state.completedText.length +
                  this.state.incorrectArr.length)) *
              100
            ).toFixed(0)
          ),
          incorrect: false,
    ...
    if (
      this.state.generatorFocus === true &&
      e.key === "Enter" &&
      findDOMNode(this.refs.sidebar.refs.genInput).value !== ""
    ) {
    ...
    return (
      <div className="App">
        ...
        <Sidebar
          showMenu={showMenu}
          displayText={this.displayText}
          genBlur={this.genBlur}
          genFocus={this.genFocus}
          ref="sidebar"
        />
      </div>
    );
